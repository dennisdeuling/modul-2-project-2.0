...
router.get('/search', (req, res, next) => {
        let {city, guests} = req.query;
        let dataQuery;
        const session = req.session;

        if (city) {
            const cityUppercase = city[0].toUpperCase() + city.slice(1).toLowerCase();
            city = cityUppercase;
        }

        if (city && guests) {
            dataQuery = {$and: [{city: city}, {guests: {$gte: guests}}]};
        } else if (city) {
            dataQuery = {city: city};
        } else if (guests) {
            dataQuery = {guests: {$gte: guests}};
        }
